...
export type NavigatorType = 'chats' | 'sources' | 'skills' | 'settings' | 'lab'
...
const COMPOUND_ROUTE_PREFIXES = [
  'allChats', 'flagged', 'state', 'label', 'view', 'sources', 'skills', 'settings', 'lab'
]
...
export function parseCompoundRoute(route: string): ParsedCompoundRoute | null {
  ...
  if (first === 'lab') {
    ...
    if (segments[1] === 'project' && segments[2]) {
      return {
        navigator: 'lab',
        details: { type: 'lab-project', id: segments[2] },
      }
    }
...
export function buildCompoundRoute(parsed: ParsedCompoundRoute): string {
  ...
  if (parsed.navigator === 'lab') {
    if (!parsed.details) return 'lab'
    return `lab/project/${parsed.details.id}`
  }
...
function convertCompoundToViewRoute(compound: ParsedCompoundRoute): ParsedRoute {
  ...
  if (compound.navigator === 'lab') {
    if (!compound.details) {
      return { type: 'view', name: 'lab', params: {} }
    }
    return { type: 'view', name: 'lab-project', id: compound.details.id, params: {} }
  }
...
function convertCompoundToNavigationState(compound: ParsedCompoundRoute): NavigationState {
  ...
  if (compound.navigator === 'lab') {
    ...
    return {
      navigator: 'lab',
      details: { type: 'lab-project', projectId: compound.details.id },
    }
  }
...
export function buildRouteFromNavigationState(state: NavigationState): string {
  ...
  if (state.navigator === 'lab') {
    if (state.details?.type === 'lab-project') {
